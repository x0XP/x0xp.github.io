...
function getClosestMatch(target) {
    ...
    for (let i = 0; i < keys.length; i++) {
        ...
        if (target.startsWith(firstWord)) return itemMap[itemName];
    }
...
async function getPrice(name, skipHistory = false) {
    ...
    // Skeleton Loader
    priceBox.innerHTML = `<div class="skeleton" style="height: 110px; margin-bottom: 5px;"></div>`;
    
    const itemData = itemMap[name.toLowerCase()];
    if (!itemData) { priceBox.innerHTML = `<div style="padding:10px; text-align:center;">Item mapping missing.</div>`; return; }
